...
volatile bool bt_notif_enabled = 0;
...
BT_GATT_SERVICE_DEFINE(remote_srv,
                       ...
                       BT_GATT_CHARACTERISTIC(BT_UUID_IMU_PREDICTIONS,
                       ...
                       BT_GATT_CHARACTERISTIC(BT_UUID_IMU_MESSAGE,
...
void button_chrc_ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
    bt_notif_enabled = (value == BT_GATT_CCC_NOTIFY);
    LOG_INF("Notifications %s", bt_notif_enabled ? "enabled" : "disabled");
    if (remote_service_callbacks.notif_changed)
    {
        remote_service_callbacks.notif_changed(bt_notif_enabled);
    }
}
